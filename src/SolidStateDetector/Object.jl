...
function find_closest_gridpoint(point::CylindricalPoint{T}, grid::CylindricalGrid{T}) where {T <: SSDFloat}
    return Int[searchsortednearest( grid.r, point.r), searchsortednearest(grid.φ, point.φ), searchsortednearest(grid.z, point.z)]
end
...
function find_closest_gridpoint(point::CartesianPoint{T}, grid::CartesianGrid{T}) where {T <: SSDFloat}
    return Int[searchsortednearest( grid.r, point.x), searchsortednearest(grid.y, point.y), searchsortednearest(grid.z, point.z)]
end
